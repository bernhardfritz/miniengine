...
export default async function (gl: WebGL2RenderingContext) {
  ...
  const vertexArrayInfo = createVertexArrayInfo(
    gl,
    programInfo,
    await createHeightmapBufferInfo(gl, 'heightmap.png', 4, 4, 255, 255),
  );
...
async function createHeightmapBufferInfo(gl: WebGLRenderingContext, src: string, width = 1, depth = 1, subdivisionsWidth = 1, subdivisionsDepth = 1, matrix?: m4.Mat4): Promise<BufferInfo> {
  const image = await loadImage(src);

  const arrays = primitives.createPlaneVertices(width, depth, subdivisionsWidth, subdivisionsDepth, matrix);
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    const [r, g, b] = [image.data[idx + 0], image.data[idx + 1], image.data[idx + 2]];
    const avg = (r + g + b) / 3;
    const y = avg / 255;
    positions[i + 1] = y;
  }
